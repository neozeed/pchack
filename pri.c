...
register xx,yy;
	...
	if(cansee(x,y)) {
		...
		tc[cnt].x = x;
		tc[cnt].y = y;
		if(++cnt >= COLNO) panic("Tmp_at overflow?",0,0,0,0,0,0);
...
#ifdef DGK
/* Here are two kinds of faster redraw -dgk
 *	1) Using BIOS call 0x10 for cursor and printing (may not be portable)
 *	2) Using a line buffer with ANSI for a speed up of about 50%
 */
	if (flags.BIOSok) {
		fflush(stdout);			/* make the cls() occur */
		for(y = 0; y < ROWNO; y++)
			for(x = 0; x < COLNO; x++)
				if((room = &levl[x][y])->new) {
					room->new = 0;
					fastat(x, y, room->scrsym);
				} else if(room->seen)
					fastat(x, y, room->scrsym);
	} else {
		for(y = 0; y < ROWNO; y++) {
