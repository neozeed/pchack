...
dosave0(hu) int hu; {
	...
	if (!saveDiskPrompt(0))
		return 0;
	/* Check to make sure there is enough space on the save disk.
	 * The extra 20000L is a conservative estimate of the amount
	 * of space necessary for the player's inventory plus an
	 * additional amount in case this level is not yet on disk.
	 * It would be much better to actually count the space necessary.
	 */
	free = freediskspace(SAVEF);
	needed = all_files_size(lock);
	if (free < needed) {
		pline("There is not enough disk space to save your game.");
		return 0;
	} else if (free < needed + 20000L) {
		pline("There may not be enough disk space to save your game.");
		pline("Do you want me to try saving the game anyway [y/n] ?");
		fflush(stdout);
		if (getchar() != 'y') {
			docrt();
			return 0;
...
register fd;
{
	...
	free = freediskspace(lock);
	needed = filesize(SAVEF);
	if (free < needed + 10000L) {
		fprintf(stderr, "\n%s\n%s\n%s",
		"There may not be enough disk space to restore your game.  If I",
		"panic while restoring your game, the save file will NOT be deleted.",
		"Should I try to restore your game [y/n] ?");
		if (getchar() != 'y') {
			settty("Be seeing you ...\n");
			exit(0);
		}
	...
	putchar('\n');
	cl_end();
	printf("You got as far as level %d%s.\n", maxdlevel,
