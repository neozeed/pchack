...
char *argv[];
{
	...
	if (getcwd(orgdir, sizeof orgdir) == NULL) {
		fprintf(stderr, "hack: current directory path too long\n");
		_exit(1);
	}
#endif MSDOS
#endif CHDIR
...
#ifdef DGK
	if (dir != NULL)
		(void) strncpy(hackdir, dir, PATHLEN);
	else
...
#ifndef DGK
	/* This is no longer needed as the options are initialized
	 * above -dgk
	 */
	/*
...
#ifndef DGK
		/* lock is set in read_config_file -dgk */
		if(!locknum)
...
not_recovered:
	...
	initrack();
#ifdef MSDOS
	/* Help the Microsoft optimizer.  Otherwise main is too large -dgk*/
	moveloop();
}
...
boolean wr;
{
	...
	if(wr) {
		...
		strcpy(tmp, dir);
		append_slash(tmp);
		printf("Warning: cannot write %s%s", tmp, RECORD);
#else
