...
#ifdef MSDOS
	/* Indices into the objname[] were stored instead of the actual
	 * pointers.  Replace the indices with pointers here.
	 */
	ObjectIndicesToPointers();
#endif MSDOS
...
unsigned len;
#ifdef MSDOS
	/* Before writing the objects, replace pointers into objname[] with
	 * indices.
	 */
	ObjectPointersToIndices();
#endif MSDOS
...
#ifdef MSDOS
	/* After reading the objects, replace indices into objname[] with
	 * pointers.
	 */
	ObjectIndicesToPointers();
#endif MSDOS
...
{
	int i;
	/* the objects have indices into the object_name and object_descr
	 * arrays.  Replace the indices with the true pointer value
	 */
	for (i = 0; i < SIZE(objects); i++) {
		if (objects[i].oc_name != NULL)
			objects[i].oc_name =
				objname[(int) (objects[i].oc_name)].name;
		if (objects[i].oc_descr != NULL)
			objects[i].oc_descr =
				objname[(int) (objects[i].oc_descr)].descr;
	}
}
...
{
	int i,j;
	/* Before writing the objects, replace pointers to object_name and
	 * object_descr with indices.
	 */
	for (i = 0; i < SIZE(objects); i++) {
		if (objects[i].oc_name != NULL)
			for (j = 0; j < SIZE(objname); j++)
				if (objects[i].oc_name == objname[j].name) {
					*((int *)(&objects[i].oc_name)) = j;
					break;
				}
		if (objects[i].oc_descr != NULL)
			for (j = 0; j < SIZE(objname); j++)
				if (objects[i].oc_descr == objname[j].descr) {
					*((int *)(&objects[i].oc_descr)) = j;
					break;
				}
	}
}
